...
export const CommandRuleSchema = z.object({
  ...
  pattern: z.string().required()
    .description('Whole-command pattern, e.g. `git *`, `git status*`, `rm -rf *`. Supports `*` (any run of characters) and `?` (exactly one). The program token is matched after normalizing (path, quotes and `.exe`-style suffixes stripped), so `git *` also matches a quoted full path to git.exe.'),
...
export class AllowlistPolicyService extends SandboxPolicyService {
  ...
  constructor(ctx, config) {
    ...
    this._sessionCache = new SessionDecisionCache({
      enabled: this.commands?.sessionCache !== false,
    })
  ...
  _registerCommandGate(ctx) {
    try {
      ctx.inject(['tools'], () => {
        ...
        applyCommandGate(ctx, shared)
        applyApprovalGate(ctx, { ...shared, proposals: this._proposals })
      })
    } catch (error) {
  ...
  _commandContext() {
    return {
      roots: [this._workspaceRoot, ...this.expandRoots()],
   ...
   * prompt. Rendered only when at least one proposal exists.
   */
  ...
  _registerSettingsNamespace(ctx) {
    try {
      ctx.inject(['settings'], (scope) => {
        ...
        // The session cache's enablement is a live setting too: without this
        // the toggle in the settings page would appear to do nothing until a
        // server restart.
        this._sessionCache.enabled = this._currentCommands()?.sessionCache !== false
