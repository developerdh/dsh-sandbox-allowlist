...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const DIRS_HINT = '支持通配符：D:\\Shared\\**（子树）、D:\\Data\\*（一级子目录）、D:\\Archive\\202?（单字符）。';
    ...
    const COMMANDS_HINT = '命令模式支持 <code>*</code>（任意多字符）与 <code>?</code>（单字符），如 <code>git status*</code>；' +
    ...
    const COMMANDS_TAIL_HINT = '空白行不会保存；工具留空（任意）时规则对所有 shell 工具生效。写工作区之外的路径请用「授权目录」，命令规则覆盖不了越界写入。';
    const COMMANDS_ESCALATION_HINT = '沙箱升级 = 命令被沙箱拒绝后，AI 带 sandbox_permissions 重试、命令将在沙箱外运行的那次审批。' +
      '开启后：命中 allow 规则的命令、以及只读/只写工作区与授权目录内路径的命令（内置能力类），升级自动放行；' +
      '关闭后：所有升级都弹审批。越界写入、禁读目标、deny/ask 命中永远不自动放行。';
    const COMMANDS_BASELINE_HINT = '开启后，只读命令与「只写工作区/授权目录内路径」的命令无需任何规则即可识别（这是内置基线，不是放行升级）。关闭后完全按你写的规则判定。';
    const COMMANDS_SESSION_HINT = '开启后，你手工批准过的某条命令（完全相同的命令文本）在本会话内不再重复询问——AI 反复重试同一条命令时只问一次。';

    const NOREAD_HINT = 'pattern 支持 <code>*</code>（任意多个字符）与 <code>?</code>（单个字符）。' +
    ...
    const NOREAD_TAIL_HINT = '空 pattern 的行不会保存。不提供 allow 动作——官方默认本就允许读；' +
      ...
      return function CommandsCard() {
        ...
        var bodyChildren = [
          ...
          react.createElement('div', { key: 'field-escalation', className: 'sabx-field' },
            react.createElement('div', { className: 'sabx-field-head' },
              react.createElement('span', { className: 'sabx-field-label' }, '沙箱升级'),
            ),
            react.createElement('label', { className: 'sabx-check' },
              react.createElement('input', {
                type: 'checkbox',
              ...
              react.createElement('span', null, '允许沙箱升级自动放行'),
            ),
            react.createElement('p', { className: 'sabx-field-hint' }, COMMANDS_ESCALATION_HINT),
          ),
          react.createElement('details', { key: 'field-advanced', className: 'sabx-advanced' },
            react.createElement('summary', null, '高级'),
            BASELINE_OPTIONS.map(function mapAdvanced(option) {
    ...
    function makeNoReadCard(scope) {
      return function NoReadCard() {
        ...
        var countText = String(saved.length) + ' 条规则';
        if (pendingCount > 0) countText += ' · 未保存修改';
        ...
        var ruleRows = rules.map(function mapNoReadRow(rule, ruleIndex) {
          return react.createElement('div', { key: ruleIndex, className: 'sabx-rule-row sabx-noread-row' },
            ...
            react.createElement('div', { className: 'sabx-rule-action' },
              createSeg(NOREAD_ACTIONS, rule.action, function onAction(value) { setRule(ruleIndex, { action: value }); }, '动作', true),
            ),
            react.createElement('button', {
              className: 'sabx-icon-btn sabx-rule-remove',
              type: 'button',
        ...
        var bodyChildren = [
          react.createElement('div', { key: 'field-rules', className: 'sabx-field' },
            react.createElement('div', { className: 'sabx-field-head' },
              react.createElement('span', { className: 'sabx-field-label' }, '限制读取的文件模式（命中即按动作处理）'),
            ),
            react.createElement('p', { className: 'sabx-field-hint', dangerouslySetInnerHTML: { __html: NOREAD_HINT } }),
            react.createElement('div', { className: 'sabx-rules-table' }, ruleRows.length > 0 ? ruleRows : [
              react.createElement('div', { key: 'empty', className: 'sabx-empty' }, '尚无禁读规则。点击「添加规则」新增一行。'),
            ]),
            react.createElement('button', { className: 'sabx-add-row', type: 'button', disabled: saving, onClick: addRule },
              react.createElement('span', { 'aria-hidden': true }, '＋'), ' 添加规则'),
            react.createElement('p', { className: 'sabx-dir-hint-inline', style: { marginTop: 4 } }, NOREAD_TAIL_HINT),
          ),
          react.createElement('div', { key: 'footer', className: 'sabx-card-footer' },
            error === null
              ? null
    ...
    function makeAllowlistSection(scope) {
      ...
      var NoReadCard = makeNoReadCard(scope);
