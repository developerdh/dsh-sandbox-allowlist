...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var INLINE_CSS = [
      ...
      /* ---- 脚注 ---- */
      '.sabx-prose-sm{color:var(--dsw-alias-label-tertiary,#9ca1a9);font-size:12px;line-height:1.55;margin:0;}',
      '.sabx-kbd{font-family:ui-monospace,"Cascadia Code",Consolas,"Courier New",monospace;font-size:11px;border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.11));background:var(--dsw-alias-bg-layer-3,#222227);border-radius:5px;padding:0 5px;color:var(--dsw-alias-label-secondary,#c9ccd0);}',
      /* ---- 窄屏折行 ---- */
    ...
    const CALL_WARN_DIRS = '安全警示：授权目录会被沙箱内的 AI 代理<b>无审批</b>写入（工作区之外）。这放宽了沙箱边界——只添加完全信任、已存在且归当前用户所有的目录，别把非受信位置加进来。';
    const DIRS_HINT = '支持通配符：D:\\Shared\\**（子树）、D:\\Data\\*（一级子目录）、D:\\Work\\202?（单字符）。只添加完全信任、已存在且归当前用户所有的目录。';
      ...
      return function AllowlistSection() {
        return react.createElement('section', { className: 'sabx-section', 'aria-labelledby': 'sabx-section-title' },
          ...
          react.createElement('p', { className: 'sabx-section-intro' },
            ...
            react.createElement('b', null, '保存'),
            '后立即生效（设置文档热加载），无需重启 dsh。',
          ),
          react.createElement(DirsCard, null),
          react.createElement(CommandsCard, null),
          react.createElement('p', { className: 'sabx-prose-sm' },
            '提示：撤销授权后，已物化到目录 DACL 的 ACE 不会被自动移除（Windows）；如需清理请用',
            react.createElement('span', { className: 'sabx-kbd' }, 'scripts/revoke.mjs'),
            '。注意不要在本分节内使用',
            react.createElement('span', { className: 'sabx-kbd' }, 'pnpm install'),
            '全局重装 web profile（github git+ssh 依赖会失败）。',
          ),
        );
      };
    }
